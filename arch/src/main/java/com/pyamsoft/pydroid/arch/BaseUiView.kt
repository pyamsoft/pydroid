...
protected constructor(parent: ViewGroup) : UiView<S, V>() {
  ...
  /**
   * Adopt the view to this UiView parent Otherwise the child view will inflate into the same parent
   * instead of the parent layoutRoot
   */
  ...
  private fun inflateAndAddToParent(inflater: LayoutInflater, parent: ViewGroup) {
   ...
   *
   * Nesting a UiView will make its parent element become this UiView's layoutRoot. This will allow
   * you to embed UiViews inside of other UiView objects.
   *
   ...
   *
   * Lazily fetches the Android View at first access and correctly releases it when the UiView is
   * torn down.
   */
  @CheckResult
  @UiThread
  ...
  protected data class Bound<V : View>
  internal constructor(private var parent: ViewGroup?, private var resolver: ((View) -> V)?) :
