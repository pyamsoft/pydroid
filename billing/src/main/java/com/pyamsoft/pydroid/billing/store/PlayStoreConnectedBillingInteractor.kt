...
internal class PlayStoreConnectedBillingInteractor
internal constructor(
  ...
  private fun querySkus() {
    ...
    // be re-used.
    val skus = skuList.map { sku ->
      Product.newBuilder().setProductType(ProductType.INAPP).setProductId(sku).build()
    }
  ...
  companion object {
    ...
    private fun BillingResult.isUserCancelled(): Boolean {
      return this.responseCode == BillingResponseCode.USER_CANCELED
    }

    @JvmStatic
    @CheckResult
    private fun BillingResult.isOk(): Boolean {
      return this.responseCode == BillingResponseCode.OK
    }
  }
}
