...
object AnimUtil {
    ...
    private fun getOvershootInterpolator(context: Context): Interpolator {
        if (overshootInterpolator == null) {
            overshootInterpolator = AnimationUtils.loadInterpolator(
    ...
    private fun getAccelCubicInterpolator(context: Context): Interpolator {
        if (accelCubicInterpolator == null) {
            accelCubicInterpolator = AnimationUtils.loadInterpolator(
                context.applicationContext,
    ...
    fun popShow(v: View, startDelay: Int, duration: Int): ViewPropertyAnimatorCompat {
        ...
        return ViewCompat.animate(v).alpha(1f).scaleX(1f).scaleY(1f).setStartDelay(
            startDelay.toLong()
        ).setDuration(duration.toLong()).setInterpolator(i).setListener(
    ...
    fun fadeIn(v: View): ViewPropertyAnimatorCompat {
        ...
        return ViewCompat.animate(v).alpha(1f).scaleX(1f).scaleY(1f).setStartDelay(300).setDuration(
            900
        ).setInterpolator(i).setListener(null)
    }
