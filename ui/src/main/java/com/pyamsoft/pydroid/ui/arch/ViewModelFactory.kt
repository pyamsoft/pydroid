...
@PublishedApi
internal class ViewModelFactoryImpl<T : UiStateViewModel<*>>
private constructor(
    type: Class<T>,
    store: ViewModelStore?,
  ...
  @Volatile private var modelResolver: (() -> T)? = null
            ...
            fragment != null -> {
              ...
              when (fragment) {
                ...
                throw IllegalArgumentException(
                    "Unable to create model resolver - ViewModelStore, Activity, and Fragment are NULL")
          }.get(type)
        }
  }
  ...
  private fun resolveValue(): T {
    val resolver =
        modelResolver ?: throw IllegalStateException("Cannot resolve ViewModel - resolver is NULL")
...
internal sealed class FragmentFactoryProvider(internal val fragment: Fragment) {
  ...
  @PublishedApi
  internal class FromFragment @PublishedApi internal constructor(fragment: Fragment) :
      FragmentFactoryProvider(fragment)

  @PublishedApi
  internal class FromActivity @PublishedApi internal constructor(fragment: Fragment) :
      FragmentFactoryProvider(fragment)
}
