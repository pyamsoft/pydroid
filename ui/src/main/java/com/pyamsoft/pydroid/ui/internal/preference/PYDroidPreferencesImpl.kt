...
internal constructor(
  ...
  override suspend fun showRating(): Boolean =
      withContext(context = Dispatchers.IO) {
        ...
        // If it has been at least a month, then when we add the date to last seen it will still be
        // before today
        val adjustedLastSeenCalendar =
            Calendar.getInstance().apply {
  ...
  override suspend fun markRatingShown() =
      withContext(context = Dispatchers.IO) {
        Enforcer.assertOffMainThread()
  ...
  companion object {
    ...
    private val lastShownDateFormatter =
